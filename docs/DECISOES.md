...
## D16. A janela do trabalho esconde o enunciado no servidor, e libera só a formação de equipes

**Decisão:** `janela_inicio` deixou de ser apenas metadado de métricas e virou trava de visibilidade. Antes dela, `GET /me/turmas` devolve o trabalho com `liberado: false`, `status: 'agendado'`, `descricao_md: null` e `deadline: null` — o aluno vê que o trabalho existe, de que tipo é e quando abre. Criar repositório (individual ou de equipe) responde **403** (`TrabalhoNaoLiberadoError`, `src/lib/janela.ts`). **Formar equipe continua liberado** — é a única ação disponível antes da abertura.

**Contexto:** professores marcam o lab com antecedência (a rotina de `OPERACAO.md §2` cadastra os trabalhos do semestre de uma vez), mas o enunciado só deve valer a partir da aula. Como `janela_inicio` não travava nada, cadastrar o trabalho equivalia a publicá-lo: o enunciado inteiro saía em `/me/turmas` e qualquer aluno já podia gerar o repositório do template.

**Justificativa:** o corte é **no payload, não na tela**. Esconder `descricao_md` só no front deixaria o enunciado a um `curl`/DevTools de distância — e um sistema de integridade acadêmica que confia no cliente para esconder a prova não tem como pedir confiança nos seus sinais. Pelo mesmo motivo a trava do repositório vive em `src/services/repo.ts` (as duas funções de criação), não nos handlers: o repositório é gerado do template do enunciado, criá-lo antes entregaria justamente o que a janela esconde.

**Por que equipe é a exceção:** equipes são por trabalho (`Equipe.trabalho_id`), então montar o grupo é preparo que não depende do enunciado — e é o que os alunos fazem naturalmente antes da aula. Bloquear isso junto tornaria a janela um custo sem contrapartida. Em outro trabalho, outros grupos podem ser formados; dentro do mesmo trabalho, um aluno só pode estar em um grupo (`createTeam`/`addTeamMember` checam por `equipe.trabalho_id`) — dois grupos seriam dois repositórios reivindicando a mesma entrega.

**Escape hatch deliberado:** um repositório já criado destrava o trabalho para o dono dele mesmo que `janela_inicio` seja reagendada para o futuro. Sem isso, um professor que corrige a data escondería o trabalho de quem já está trabalhando nele.

**Rejeitado:** esconder também o título (o aluno precisa identificar o trabalho para montar a equipe certa — o quadro de equipes é por trabalho); e criar um campo novo `publicado_em` em vez de reusar `janela_inicio` (a janela já era o par abertura/prazo, um terceiro instante só produziria estados inconsistentes entre "aberto" e "publicado").
