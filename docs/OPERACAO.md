...
| Aluno clica em criar repositório e recebe `Failed to generate repository on GitHub: Not Found`, mas o template abre normalmente no navegador | O `/generate` devolve 404 para quatro causas: repositório sem a caixinha **Template repository** (a mais comum), template invisível para a instalação (Only select repositories, ou dono diferente da org), `Administration: write` pendente de aprovação do owner, App não instalado | `npx tsx scripts/diagnostico-template.ts owner/repo` (só leitura) aponta qual das quatro é. Desde a checagem no POST/PATCH de trabalho, o caso "não é template" é barrado na criação do trabalho, com o link do Settings na mensagem |
