...
**Repo setup is a job, not inline work.** `createRepositoryFor{Student,Team}` generates the repo from the template, persists the row, and enqueues `repo-setup`. The worker calls `configureRepository()` (poll for `main`, add collaborators, protect the branch) with 5 attempts and exponential backoff; when the retries are exhausted it writes `setup_status = ERRO` plus `setup_erro` on the `Repositorio`. That status is surfaced to professors in the grade rows and in `getRepositoryMetrics`, so a student left without push access is visible rather than buried in logs. `POST /prof/repositorios/:id/reprocessar-setup` resets a repo stuck in `ERRO` back to `PENDENTE` and re-enqueues the job — the ruleset step is idempotent (checks by name before creating), so reprocessing an already-partially-configured repo is safe. **Só o passo essencial (adicionar colaboradores) define `ERRO`; aplicar o ruleset de proteção é best-effort** — numa org Free com repo privado ele nunca é aplicável (403 de plano), e marcar `ERRO` um repo que funciona só assustava. O que sustenta a integridade nesse caso é o detector de force-push (detecção pós-fato), não a prevenção. Ver a correção em `docs/DECISOES.md` D14 e `isPlanLimitation` em `src/services/repo.ts`.
...
- **Repo creation** (`src/services/repo.ts`) generates from the trabalho's `template_repo`, then polls up to ~20s for the `main` branch to materialize before adding collaborators and creating the `protect-main` Repository Ruleset (not classic branch protection — see `docs/DECISOES.md` D14, the org is on GitHub Free).
...
- **O template é conferido antes de gravar o trabalho** (`checarTemplateNoGithub`, usado pelo POST e pelo PATCH): o repositório precisa existir para o token da instalação, ter `is_template` e não estar arquivado. Sem essa checagem o erro só apareceria depois, como um `404 Not Found` do `/generate` para o primeiro aluno — e esse 404 é a mesma resposta para quatro causas distintas. `scripts/diagnostico-template.ts` separa as quatro quando o problema já está em produção.
- **`PATCH /prof/trabalhos/:id` é parcial e tem dois travamentos.** `turma_id` não é aceito (repositórios, equipes e entregas ficariam na turma antiga) e `tipo` só muda enquanto `repositorio.count` do trabalho for zero — trocar depois deixaria repos `ALUNO` num trabalho `EQUIPE`. O `template_repo` é revalidado no GitHub apenas quando muda, e a coerência `deadline > janela_inicio` é checada contra o valor do banco, já que o PATCH pode mexer só numa ponta. Editar o template **não regera** repositório existente: vale para quem ainda não criou o seu.
- **`janela_inicio` é uma trava, não metadado.** Antes dela o trabalho está *agendado*: `GET /me/turmas` devolve `liberado: false`, `status: 'agendado'`, `descricao_md: null` e `deadline: null`, e as duas funções de criação de repositório (`src/services/repo.ts`) lançam `TrabalhoNaoLiberadoError` (403, `src/lib/janela.ts`). O corte é no payload — esconder o enunciado só na tela o deixaria a um `curl` de distância. **Formar equipe é a única coisa liberada antes da abertura**; ao mexer nessas rotas, não estenda a trava para `POST /trabalhos/:id/equipes`, `POST /equipes/:id/membros` ou `GET /trabalhos/:id/equipes`. Um repositório já existente destrava o trabalho para o dono, para que reagendar a janela não esconda o trabalho de quem já o começou. Ver `docs/DECISOES.md` D16.
- **Um aluno, uma equipe por trabalho.** `createTeam` e `addTeamMember` (`src/services/team.ts`) recusam quem já está em outro grupo **do mesmo trabalho** — o filtro é `equipe: { trabalho_id }`, nunca global: em outro trabalho o mesmo aluno monta outro grupo, e é assim que deve ser.
- **Team listing is privacy-scoped.** `GET /trabalhos/:id/equipes` (`listTeamsForTrabalho` em `src/services/team.ts`) alimenta o "quadro da turma" do aluno no front. Devolve apenas `total_integrantes` e `status` (`completo` = já criou repositório) por equipe — **nunca a composição dos grupos** —, mais `sou_membro` para a UI destacar a equipe do próprio aluno, e só para alunos matriculados na turma. Ao estender, não passe a expor os membros: a ausência deles é intencional.
...
- `tests/janela_trabalho.test.ts` asserts the closed trabalho does not ship `descricao_md`/`deadline` at all — e que formar equipe segue liberado nesse estado.
